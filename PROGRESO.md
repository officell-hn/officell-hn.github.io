...
| Fecha | Cambio |
|-------|--------|
| 2026-09-17 | Revisión semanal admin. **Auth 100% JWT Bearer en los 3 paneles, sin `x-admin-token`; 0 referencias DOM rotas (cruzado `getElementById()` vs `id=`), 0 variables CSS sin resolver; sin código muerto ni funciones duplicadas.** Desde el ciclo anterior no cambió código de paneles (solo fichas y sitemap). **Bug de presentación corregido en `tienda.html`:** `abrirCheckout()` interpolaba el nombre del producto **sin escapar** (`${i.nombre}`) al armar el resumen del checkout — único punto de la página donde un nombre entra a `innerHTML` sin `escHtml()` (el carrito y el catálogo sí escapan). Un producto con `<`, `>`, `&` o `"` en el nombre se veía bien en el carrito pero **mangleado en la pantalla de pago**. Fix: `${escHtml(i.nombre)}`, alineado con `renderCarrito()`. Las apariciones del nombre en `confirmarPedido()` (texto de WhatsApp y JSON del pedido) no tocan HTML y se dejaron igual. Detalle en `MEJORAS_WEB.md`. Sin pendientes abiertos en Web. |
| 2026-09-10 | Revisión semanal admin. **Auth 100% JWT Bearer en los 3 paneles, sin `x-admin-token`; 0 CSS/DOM rotos; sin código muerto ni funciones duplicadas.** Desde el ciclo anterior no cambió código de paneles (solo fichas y sitemap). **Bug de UX/datos corregido en `admin-productos.html`:** los switches de la tabla de Inventario (En Web / Destacado / Venta permanente) **no revertían el checkbox cuando fallaba el guardado** — el `onchange` corre tras el flip del navegador, así que ante error de red o rechazo del backend el switch quedaba pintado en el estado nuevo y solo un toast de 3 s avisaba; un producto podía verse "En Web" sin estar publicado (o al revés). Fix: `toggleCampo()` recibe el checkbox (`this`) y lo revierte en el `catch`. Detalle en `MEJORAS_WEB.md`. Sin pendientes abiertos en Web. |
| 2026-09-03 | Revisión semanal admin. **Auth 100% JWT Bearer en los 3 paneles, sin `x-admin-token`; 0 CSS/DOM rotos; sin código muerto ni funciones duplicadas.** Desde el ciclo anterior no cambió código de paneles (solo fichas, sitemap y fotos ZTE/Laver). **Bug de lógica corregido en `tienda.html`:** el botón **+** del carrito (`cambiarQty`) subía la cantidad **sin respetar el stock**, anulando parcialmente el guard de agotados del 9-ago — un cliente podía pedir más unidades de las que hay. Causa: `{...producto, cantidad:1}` pisaba el stock del producto, dejando `cambiarQty` sin dato para validar. Fix: se guarda el stock aparte (`_stock`) al agregar y `cambiarQty` topa contra él al incrementar; degradación limpia para carritos viejos de localStorage y el backend revalida. `_stock` no se filtra al pedido ni a la UI. Detalle en `MEJORAS_WEB.md`. Sin pendientes abiertos en Web. |
| 2026-08-27 | Revisión semanal admin. **Sin bugs críticos ni de lógica JS** (auth 100% JWT Bearer en los 3 paneles, sin `x-admin-token`; 0 CSS/DOM rotos; sin código muerto ni funciones duplicadas). Hardening aplicado: **escape del preview de imágenes en `admin-productos.html`** (`actualizarPreviews()`, `src="${escHtml(url)}"`) — hueco gemelo del fix del 2026-07-16, que escapó el `value` del input pero dejó cruda la función hermana que relee esos valores y los pinta en el `<img>` del preview; una URL con `"` rompía el atributo. Con esto los 100% de los puntos donde una URL se interpola en HTML pasan por `escHtml()`. Detalle en `MEJORAS_WEB.md`. Sin pendientes abiertos en Web. |
| 2026-08-17 | **Export CSV de pedidos** en `admin-productos.html` (botón ⬇️ CSV + `exportarCSVPedidos()`) — cierra M2, pendiente desde 2026-07-30. **Con esto las tres pestañas del panel exportan a CSV** (Taller 12-jun, Inventario 13-ago, Pedidos hoy). Respeta la búsqueda **y** el toggle "Ver cancelados"; no reusa `pedidosFiltrados` porque `renderPedidos()` la guarda antes de descartar los cancelados y habría exportado pedidos que no están en pantalla. Estados en texto plano sin emoji y fecha `YYYY-MM-DD HH:MM:SS` en hora de Honduras, para que Excel ordene y un contador lo pueda leer. Probado con 21 casos sobre la función real (filtros, escape de comas, `productos` roto o como texto JSON, fecha nula, respaldo a `paypal_order_id`). |
...
- [x] Exportar pedidos a CSV — implementado 2026-08-17 (botón ⬇️ CSV, `exportarCSVPedidos()`, respeta búsqueda y "Ver cancelados")
