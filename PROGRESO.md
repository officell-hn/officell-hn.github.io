...
| Fecha | Cambio |
|-------|--------|
| 2026-09-24 | Revisión semanal admin. **Auth 100% JWT Bearer en los 3 paneles, sin `x-admin-token`; 0 CSS/DOM rotos; sin código muerto ni funciones duplicadas.** Paginación, filtros, export CSV/PDF y spinners consistentes; QR local en tienda y checkout con doble-submit/rechazo-de-stock/caída-de-red ya cubiertos. **Bug de robustez corregido en `admin-keyson.html`:** `cargarClientes()` y `abrirConversacion()` hacían `!d.clientes.length` / `!d.mensajes.length` **sin verificar que el arreglo existiera** — si el backend responde `{ok:true}` sin la clave, `.length` lanza `TypeError`, cae en el `catch` y el panel muestra "Error cargando…" en vez del estado vacío correcto ("Sin conversaciones" / "Sin mensajes"). Fix: `Array.isArray(...)` antepuesto en ambos guards. Detalle en `MEJORAS_WEB.md`. Sin pendientes abiertos en Web. |
