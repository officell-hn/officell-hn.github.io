...
| Fecha | Cambio |
|-------|--------|
| 2026-08-06 | Revisión semanal. **🔴 CRÍTICO detectado (requiere acción del dueño):** el commit `8afa59a` cableó enlaces a `producto/{slug}.html` en `tienda.html` + `sitemap.xml` pero **nunca commiteó las 11 páginas generadas** → todo enlace de producto da 404 y Google recibía 11 URLs 404. Fix parcial seguro aplicado: vaciado el bloque de productos de `sitemap.xml` (marcadores conservados, regenerar lo rellena). Los enlaces de `tienda.html` se dejaron intactos (feature del dueño; el arreglo completo es generar+commitear las páginas con `scripts/generar_paginas_producto.js` del repo officell-ia). **2 bugs corregidos en `tienda.html`:** (1) `onerror` de la imagen principal tenía comillas escapadas que cerraban el string JS → el emoji de respaldo nunca se mostraba cuando fallaba una imagen; ahora usa `&quot;`. (2) `renderCarrito` insertaba `imgSrc` y `item.nombre` sin escapar → ahora `escHtml()`, consistente con el resto. Admin (taller/productos/keyson) revisado: auth JWT correcta, sin refs DOM rotas. Mejora pendiente documentada: export CSV en pestaña Pedidos. |
